...
class BaseStyleGenerator(nn.Module):
    def __init__(self, cfg, classnames, clip_model, device):
        ...
        self.base_embedding = clip_model.token_embedding(self.tokenized_base_text).to("cpu")  
        self.style_embedding = []  
        self.stylized_base_text_encoder_out = []  
    ...
    def _init_stylized_text(self, base_text=None, style_position=0):
        ...
        stylized_base_text_embedding = self.clip_model.token_embedding(tokenized_base_text)  
...
class GeoStylerGenerator(BaseStyleGenerator):
    def __init__(self, cfg, classnames, clip_model, device):
        super().__init__(cfg, classnames, clip_model, device)
    ...
    def reinit_style(self, embedding_dim=512):
        print("GeoStylerGenerator not need to call reinit_style")
