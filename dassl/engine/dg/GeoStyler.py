...
class GeoStyler(nn.Module):
    def __init__(self, cfg, classnames, clip_model, device):
        ...
        self.weight_save_path = os.path.join(cfg.TRAINER.GEOSTYLER.WEIGHT_DIR_PATH,
                                             cfg.TRAINER.GEOSTYLER.CHECK_POINT_NAME)
        ...
        self.train_flag = cfg.TRAINER.GEOSTYLER.TRAIN_STYLE
        # orthogonality / ETF configuration
        self.orth_mode = getattr(cfg.TRAINER.GEOSTYLER, "ORTH_MODE", "none").lower()
        self.orth_lambda = getattr(cfg.TRAINER.GEOSTYLER, "ORTH_LAMBDA", 0.0)
        self.etf_targets = None
        if self.orth_mode == "e":
            with torch.no_grad():
                dummy_out = self.clip_model.forward_text(
                    self.prompt_style_template_embedding, self.tokenized_prompt_style_template
                )
            text_dim = dummy_out.shape[1]
            rand_mat = torch.randn(text_dim, self.n_style, device=self.device, dtype=torch.float)
            q, _ = torch.linalg.qr(rand_mat)
            self.etf_targets = q.t()
    ...
    def forward(self, style_idx):
        ...
        orth_mode = getattr(self, "orth_mode", "none")
        orth_lambda = getattr(self, "orth_lambda", 0.0)

        # --------- style diversity (original text-based loss) ---------
        if style_idx > 0:
            ...
            current_prompt_output = self.clip_model.forward_text(
                current_prompt, self.tokenized_prompt_style_template
            )  # 1*1024
            with torch.no_grad():
                ...
                before_prompts_output = self.clip_model.forward_text(
                    before_prompts, before_tokenized_prompts
                )  # style_idx*1024
            # normalize
            ...
            current_prompt_output_norm_rep = current_prompt_output_norm.repeat(
                before_prompts_output_norm.size(0), 1
            )

            cos_sim = F.cosine_similarity(current_prompt_output_norm_rep, before_prompts_output_norm, dim=1)

            style_diversity_loss_orig = torch.abs(cos_sim).mean()
        else:
            prefix = self.prompt_style_template_embedding[:, :2, :]  # 1*2*512
            suffix = self.prompt_style_template_embedding[:, 3:, :]  # 1*74*512
            current_prompt = torch.cat(
            ...
            if orth_mode.lower() == "e" and self.etf_targets is not None:
                feat = current_prompt_output_norm.squeeze(0)
                etf_target = self.etf_targets[0].to(feat.device)
                style_diversity_loss_orig = F.mse_loss(feat, etf_target)
            else:
                style_diversity_loss_orig = torch.tensor(0.0, device=self.device)
            
            before_prompts_output_norm = None
            #current_prompt_output_norm = None

        # ETF loss over pure style prompt (for E scheme)
        if style_idx > 0 and orth_mode.lower() == "e" and current_prompt_output_norm is not None:
            feat = current_prompt_output_norm.squeeze(0)
            etf_target = None
            if self.etf_targets is not None and style_idx < self.etf_targets.size(0):
                etf_target = self.etf_targets[style_idx].to(feat.device)
            if etf_target is not None:
                etf_loss = F.mse_loss(feat, etf_target)
            else:
                etf_loss = torch.tensor(0.0, device=self.device)
        elif style_idx == 0 and orth_mode.lower() == "e":
            if self.etf_targets is not None:
                feat = current_prompt_output_norm.squeeze(0)
                etf_target = self.etf_targets[0].to(feat.device)
                etf_loss = F.mse_loss(feat, etf_target)
            else:
                etf_loss = torch.tensor(0.0, device=self.device, requires_grad=True)
        else:
            etf_loss = torch.tensor(0.0, device=self.device)

        # --------- optional orthogonality losses ---------

        # A: embedding-space orthogonality (over styles 0..style_idx)
        if style_idx > 0 and orth_mode.lower() in ("a", "c"):
            E = self.style_embedding[: style_idx + 1, 0, :]  # (k+1, dim)
            E_norm = F.normalize(E, p=2, dim=1)
            G = torch.matmul(E_norm, E_norm.t())
            I = torch.eye(G.size(0), device=G.device, dtype=G.dtype)
            orth_loss_A = (G - I).pow(2).mean()
        else:
            orth_loss_A = torch.tensor(0.0, device=self.device)

        # B: text-output-space orthogonality (pure style prompts)
        if style_idx > 0 and orth_mode.lower() in ("b", "d"):
            T = torch.cat([before_prompts_output_norm, current_prompt_output_norm], dim=0)
            G_text = torch.matmul(T, T.t())
            I_text = torch.eye(G_text.size(0), device=G_text.device, dtype=G_text.dtype)
            orth_loss_B = (G_text - I_text).pow(2).mean()
        else:
            orth_loss_B = torch.tensor(0.0, device=self.device)

        # choose which style_diversity_loss and orth_loss to use

        if orth_mode.lower() == "e":
            style_diversity_loss = etf_loss
            orth_loss = torch.tensor(0.0, device=self.device)
        else:
            style_diversity_loss = style_diversity_loss_orig
            orth_loss = torch.tensor(0.0, device=self.device)
        ...
        sc_prompts_output = self.clip_model.forward_text(
            sc_prompts, self.tokenized_prompt_s_c
        )  # 7*1024
        sc_prompts_output_norm = F.normalize(sc_prompts_output, p=2, dim=1)
        zimm = F.cosine_similarity(
            sc_prompts_output_norm.unsqueeze(1), self.prompt_content_output_norm.unsqueeze(0), dim=2
        )
        ...
        total_loss = style_diversity_loss + content_consistency_loss_dj
        

        return style_diversity_loss, content_consistency_loss_dj, orth_loss, total_loss
