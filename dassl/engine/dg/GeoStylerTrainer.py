...
from dassl.engine.dg.GeoStyler import GeoStyler
...
class GeoStylerTrainer(TrainerBase):
    def __init__(self, cfg):
        ...
        weight_dir_path = cfg.TRAINER.GEOSTYLER.WEIGHT_DIR_PATH
        if not os.path.exists(weight_dir_path):
            os.makedirs(weight_dir_path)
        self.weight_save_path = os.path.join(weight_dir_path, cfg.TRAINER.GEOSTYLER.CHECK_POINT_NAME)
    ...
    def build_model(self):
        ...
        self.model = GeoStyler(cfg, self.classnames, self.clip_model, self.device)
    ...
    def train(self):
        ...
        for style_idx in range(0, self.n_style):
            for epoch in range(self.max_epoch):
                ...
                # model forward
                style_diversity_loss, content_consistency_loss, orth_loss, total_loss = self.model(style_idx)
                # *********Total loss**************
                total_loss.backward()
                self.optim.step()
                self.sched.step()
                ...
                if (epoch + 1) % 20 == 0 or epoch == 0:
                    ...
                    info += [f"orth_loss {orth_loss.item()}"]
